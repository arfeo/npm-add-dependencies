...
class AddDependencies {
  constructor(dependencies = [], target = 'dependencies', overwrite = true, packageFilePath = './package.json') {
    this.result = {};
  ...
  runNpmShow(dep) {
    if (!dep) {
      console.error('\x1b[31m%s\x1b[0m', `No Dependency Provided to runNpmShow()`);
  ...
  saveToPackage() {
    return Files.readFromFile(this.packageFilePath)
      .then(async (data) => {
        ...
        return Files.writeToFile(this.packageFilePath, JSON.stringify(json, null, 2))
          .then(() => {
